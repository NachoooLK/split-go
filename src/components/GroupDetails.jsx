...
function GroupDetails({ group, categories, user, onBack, onAddExpense, getGroupBalance, getMinimalTransfers, onEditExpense, onDeleteExpense, onToggleSettled, onSettleExpense }) {
  ...
  const filteredExpenses = useMemo(() => {
    let filtered = group.expenses.filter(expense => expense.amount > 0)
    
    // Obtener nombre del usuario actual
    const currentUserName = userMetrics.memberName
    
    // Filtro por tipo
    switch (expenseFilter) {
      case 'my':
        filtered = filtered.filter(expense => expense.paidBy === currentUserName)
        break
      case 'others':
        filtered = filtered.filter(expense => expense.paidBy !== currentUserName)
        break
      case 'pending':
        filtered = filtered.filter(expense => {
          ...
          return expense.splitBetween.some(member => !settledBy.includes(member))
        })
        break
    }
  ...
  }, [group.expenses, expenseFilter, categoryFilter, userMetrics.memberName])
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-slate-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-gray-100">Filtros rápidos</h3>
          <button
          ...
          >
            ...
            <span>{showFilters ? 'Ocultar' : 'Más filtros'}</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            { key: 'my', label: 'Mis gastos', icon: CreditCard },
            { key: 'others', label: 'Otros', icon: Users },
            { key: 'pending', label: 'Pendientes', icon: AlertTriangle }
        ...
        {showFilters && (
          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-gray-600">
            <label className="block text-xs font-medium text-slate-700 dark:text-gray-300 mb-2">Categoría</label>
            <select
              value={categoryFilter}
              onChange={(e) => setCategoryFilter(e.target.value)}
              className="w-full p-2 text-sm bg-slate-50 dark:bg-gray-700 border border-slate-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-gray-100"
            >
        ...
        {(expenseFilter !== 'all' || categoryFilter !== 'all') && (
          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-gray-600 flex items-center justify-between">
            <span className="text-xs text-slate-600 dark:text-gray-400">
              {filteredExpenses.length} de {group.expenses.filter(e => e.amount > 0).length} gastos
            </span>
            <button
              onClick={() => {
      ...
      {group.expenses.length === 0 ? (
        ...
        <div className="text-center py-8">
          <div className="w-12 h-12 bg-gradient-to-r from-slate-100 to-slate-200 dark:from-gray-700 dark:to-gray-600 rounded-full flex items-center justify-center mx-auto mb-3">
            <Filter className="w-6 h-6 text-slate-500 dark:text-gray-400" />
          </div>
          <h3 className="text-md font-medium text-slate-700 dark:text-gray-300 mb-2">
            No hay gastos con estos filtros
          </h3>
          <p className="text-sm text-slate-500 dark:text-gray-400 mb-3">
            Prueba cambiando los filtros o agregando un nuevo gasto
          </p>
          <button
            onClick={() => {
              setExpenseFilter('all')
        ...
        <div className="space-y-0">
          {filteredExpenses.map((expense, index) => {
            ...
            // Verificar si es mi gasto
            const isMyExpense = expense.paidBy === userMetrics.memberName
                  ...
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        {isMyExpense && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-200">
                            Tú
                          </span>
                        )}
                        {hasUnsettled && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200">
                            Pendiente
                          </span>
                        )}
                      </div>
